...
**商群**：群G的正规子群N的全体陪集对于陪集的乘法做成一个群，记为G/N.
...
>（若使用**3.1节课后题1**结论需要证明）
>
>Proof：设N为G的n阶子群，则由3.1节结论知$aHa^{-1}\leq G$,
>
...
>$$
>H=k^{-1}Hk,k\in K\subset G\\
>K=hKh^{-1},h\in H\subset G
>$$
...
**群同态基本定理**：设$\varphi$为群G到群$\overline G$的一个同态满射，$N=Ker\varphi\unlhd G$，则：
$$
G/N\cong \overline G.
$$
...
>
>Proof：由于单群的正规子群只有{e}和其本身，又$Ker\varphi$为正规子群：
>
>若$Ker\varphi$=$\{e\}$，由群同态基本定理有：$G\cong G/Ker\varphi\cong \overline G$，即同态像同构于它自身，故为单群；
>
>若$Ker\varphi$=G，由群同态基本定理由：$\{e\}\cong G/Ker\varphi\cong \overline G$，即同态像同构于单位元群。
